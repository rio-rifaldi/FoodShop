...
type output ={ desc:string}

function Detail() {
   const {productId} = useParams()
   const {classes} = useStyles();
      const {data} = useQuery<ProductDetailI>(GET_PRODUCT_DETAIL,{
         variables : {
            productId
         }
      })
      const {AddToCart}  = useAddToCart(productId as string)
      const {handleSubmit,reset,register} = useForm<output>()
      const onsubmit = handleSubmit(({desc},e) =>{
         e?.preventDefault()
         AddToCart(desc)
         reset()
      } )
